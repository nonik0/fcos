...
void Rtc::SetDateTime(uint8_t day, uint8_t month, uint8_t weekday, uint8_t year, uint8_t hour, uint8_t minute, uint8_t second) {
...
void Rtc::CheckNTPTime() {
    if (WiFi.isConnected() && m_uptime > m_uptimeForNextNTPUpdate) {
        if (GetLocalTime(&m_timeinfo, MAX_WAIT_FOR_NTP_MS)) {
            ...
            SetDateTime(
                m_timeinfo.tm_mday, m_timeinfo.tm_mon + 1, m_timeinfo.tm_wday, m_timeinfo.tm_year - 100,
